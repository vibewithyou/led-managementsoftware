...
class SettingsService extends ChangeNotifier {
  ...
  void _ensureLoaded() {
    ...
    try {
      ...
      final items = (payload['liveActions'] as List<dynamic>? ?? const <dynamic>[])
          ...
          .toList(growable: false);
      final normalizedItems = _normalizeLegacyActionIds(items);
      ...
      _liveActionsCache = _mergeMissingDefaults(normalizedItems, seeded);
      if (!kIsWeb) _persist();
    } catch (_) {
      _liveActionsCache = seeded;
      _vlcExecutablePath = '';
  ...
  List<LiveActionConfig> _normalizeLegacyActionIds(List<LiveActionConfig> actions) {
    var hasFallback = actions.any((item) => item.id == 'fallback');
    final normalized = <LiveActionConfig>[];

    for (final action in actions) {
      if (action.id == 'fallback_default') {
        if (hasFallback) {
          continue;
        }
  ...
  String _descriptionFor(LiveActionConfig action) {
    switch (action.id) {
      ...
      case 'timeout_home':
        return 'Timeout Heimteam starten';
      case 'timeout_guest':
        return 'Timeout Gastteam starten';
      case 'timeout':
      ...
      case 'fallback':
        return 'Projekt-Fallback auslösen';
      case 'black_screen':
      ...
      case 'halftime':
        return 'Halbzeitclip starten';
      case 'game_end':
        return 'Spielende-Clip starten';
      default:
        return 'Live-Aktion';
    }
  }
