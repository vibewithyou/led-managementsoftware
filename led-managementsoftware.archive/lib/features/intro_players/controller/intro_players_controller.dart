...
class IntroPlayersController extends ChangeNotifier {
  ...
  bool get hasSelectedTeamPlayers => selectedEntries.isNotEmpty;
  bool get hasPlayableIntroSequence => selectedItems.any((item) => item.entry.isActive && item.hasClip);
  bool get canControlIntro => hasActiveProject && hasPlayableIntroSequence;
  ...
  List<IntroLineupItemModel> get allItems {
    return [..._homeEntries, ..._guestEntries]
        .map((entry) => IntroLineupItemModel(entry: entry, mediaAsset: _clipById[entry.introCueId]))
        .toList(growable: false);
  }
  ...
  Future<void> load() async {
    ...
    await _loadForProjectId(activeProject.id);
  }
  ...
  Future<void> loadForProject(String projectId) async {
    ...
    _error = null;
    notifyListeners();
    await _loadForProjectId(projectId);
  }

  Future<void> _loadForProjectId(String projectId) async {
    try {
  ...
  Future<void> addPlayer({
    ...
    final newEntry = LineupEntry(
      ...
      position: position,
      teamType: teamType,
      introCueId: mediaAssetId.trim().isEmpty ? null : mediaAssetId,
  ...
  Future<void> updatePlayer(LineupEntry updated) async {
    await _repository.saveLineupEntry(updated);
    await load();
  }
