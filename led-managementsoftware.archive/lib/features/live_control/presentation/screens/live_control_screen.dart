...
class _LiveControlScreenState extends State<LiveControlScreen> {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final gameSlots = _buildGameEventSlots();
        final flowSlots = _buildFlowEventSlots();
        final phaseAndIntroSlots = _buildPhaseAndIntroSlots();
            ...
            Expanded(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final actionFlex = compactHeight ? 3 : 3;
                  final technicalExpanded = _technicalExpanded || _controller.hasTransportError;
                  final technicalHeight = technicalExpanded ? (compactHeight ? 200.0 : 230.0) : 74.0;
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: _actionsDeck(
                          ...
                          gameSlots: gameSlots,
                          flowSlots: flowSlots,
                          phaseAndIntroSlots: phaseAndIntroSlots,
                        ),
                      ),
                      const SizedBox(height: AppSpacing.md),
                      SizedBox(
                        height: technicalHeight,
                        child: _technicalPanel(context),
                      ),
                    ],
                  );
                },
  ...
  Widget _actionsDeck(
    ...
    required List<_ActionSlot> flowSlots,
    required List<_ActionSlot> phaseAndIntroSlots,
  }) {
    final sections = [
      _ActionSectionData('Spielereignisse', 'Tor, Karten, 2 Minuten', gameSlots),
      _ActionSectionData('Unterbrechung', 'Timeout Heim/Gast, Wischer', flowSlots),
      _ActionSectionData('Spielphasen & Intro', 'Halbzeit, Spielende, Intro-Schnellzugriff', phaseAndIntroSlots),
    ];

    return AppPanel(
      title: 'Sonderclips / Eventclips',
      trailing: Wrap(
        spacing: AppSpacing.xs,
        crossAxisAlignment: WrapCrossAlignment.center,
        children: [
          OutlinedButton.icon(
            onPressed: () => Navigator.of(context).pushNamed(AppRoute.introPlayers.path),
            icon: const Icon(Icons.groups_rounded, size: 16),
            label: const Text('Intro öffnen'),
          ),
          StatusBadge(
  ...
  Widget _emergencyBar(BuildContext context) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.symmetric(horizontal: AppSpacing.md, vertical: AppSpacing.sm),
      decoration: BoxDecoration(
      ...
      child: Wrap(
        ...
        crossAxisAlignment: WrapCrossAlignment.center,
        children: [
          const StatusBadge(label: 'NOTFALLLEISTE', type: StatusBadgeType.locked, compact: true),
          FilledButton.icon(
            onPressed: _triggerEmergencyStop,
            style: FilledButton.styleFrom(backgroundColor: AppColors.warning, foregroundColor: AppColors.textPrimary),
  ...
  Widget _technicalPanel(BuildContext context) {
    final expanded = _technicalExpanded || _controller.hasTransportError;

    return AppPanel(
      title: 'Technik & Verlauf',
      trailing: Row(
        ...
        children: [
          if (_controller.hasTransportError)
            const Padding(
              ...
              child: StatusBadge(label: 'STÖRUNG', type: StatusBadgeType.error, compact: true),
            ),
          IconButton(
            onPressed: () => setState(() => _technicalExpanded = !_technicalExpanded),
            icon: Icon(expanded ? Icons.expand_less_rounded : Icons.expand_more_rounded),
            tooltip: expanded ? 'Details ausblenden' : 'Details anzeigen',
          ),
        ],
      ),
      child: AnimatedSwitcher(
        ...
        child: !expanded
            ? Align(
                ...
                child: Text(
                  'Diagnose, letzte Aktionen und Testausgabe',
                  style: Theme.of(context).textTheme.bodySmall?.copyWith(color: AppColors.textMuted),
                ),
              )
            : Column(
                key: const ValueKey('technical_expanded'),
                children: [
                  Expanded(
                    child: Row(
                      children: [
                        Expanded(child: _recentActionsPanel(context)),
                        const SizedBox(width: AppSpacing.md),
                        Expanded(child: _compactSystemStatus(context)),
                      ],
                    ),
                  ),
                  const SizedBox(height: AppSpacing.sm),
                  Wrap(
                    ...
                    children: [
                      ...
                      OutlinedButton.icon(
                        ...
                        label: const Text('Intro-Steuerung öffnen'),
                      ),
                    ],
                  ),
                ],
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        StatusBadge(
                          label: waitingReason.toUpperCase(),
                          type: _controller.sponsorLockedRunning ? StatusBadgeType.locked : StatusBadgeType.queued,
                          compact: true,
                        ),
                      ],
                    ),
  ...
  List<_ActionSlot> _buildGameEventSlots() {
    final slots = [
    ...
    return slots.where((slot) => slot.action != null && slot.onPressed != null).toList(growable: false);
  }

  List<_ActionSlot> _buildFlowEventSlots() {
    final slots = [
      _slotFromAction('timeout_home', 'Timeout Heim', semanticColor: AppColors.primary, icon: Icons.pause_circle_filled_rounded),
      _slotFromAction('timeout_guest', 'Timeout Gast', semanticColor: AppColors.primary, icon: Icons.pause_circle_rounded),
    ...
    final configured = slots.where((slot) => slot.action != null && slot.onPressed != null).toList(growable: false);

    final hasHomeOrGuest = configured.any((slot) => slot.action?.id == 'timeout_home' || slot.action?.id == 'timeout_guest');
    if (!hasHomeOrGuest) {
      return configured;
    }

    return configured.where((slot) => slot.action?.id != 'timeout').toList(growable: false);
  }

  List<_ActionSlot> _buildPhaseAndIntroSlots() {
    final slots = [
      _slotFromAction('fallback', 'Fallback', semanticColor: AppColors.secondary, icon: Icons.home_filled),
      _slotFromAction('halftime', 'Halbzeit', semanticColor: AppColors.warning, icon: Icons.sports_score_rounded),
      _slotFromAction('game_end', 'Spielende', semanticColor: AppColors.error, icon: Icons.flag_rounded),
  ...
  _ActionSlot _slotFromAction(
    ...
    return _ActionSlot(
      label: label,
      subLabel: _controller.actionCueLabel(action),
      icon: icon,
      semanticColor: semanticColor,
      emergency: emergency,
      sponsorRelevant: sponsorRelevant,
      action: action,
      onPressed: _controller.isActionConfigured(action) ? () => _controller.triggerAction(action) : null,
    );
  }
  ...
  _ActionKeyState _slotState(_ActionSlot slot) {
    ...
    final action = slot.action;
    if (action != null) {
      final isActive = _controller.isActionActive(action);
      if (isActive) {
  ...
  void _triggerEmergencyStop() {
    final stopAction = _controller.actions.where((item) => item.id == 'stop').cast<LiveActionConfig?>().firstWhere((item) => item != null, orElse: () => null);
    if (stopAction != null) {
      _controller.triggerAction(stopAction);
      return;
    }

    _controller.triggerEmergencyBlackScreen();
  }
